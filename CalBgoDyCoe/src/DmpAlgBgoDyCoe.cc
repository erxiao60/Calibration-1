...
//  gDataBuffer->LinkRootFile("Event/Rdc/Bgo",fBgoRaw);
  ...
  fBgoDyCoe->StartTime = fEvtHeader->fSecond;
...
//  std::cout<<"0000000000000000000000000"<<std::endl;
  for(short layer=0;layer<14;++layer){
	...
	else if(adc_dy5[layer][bar][side]>8000 && adc_dy5[layer][bar][side]<12000 && FitRangeDy5_h[gid_pmt]<50){
	...
	else if(adc_dy8[layer][bar][side]>8000 && adc_dy8[layer][bar][side]<12000 && FitRangeDy8_h[gid_pmt]<50){
...
bool  DmpAlgBgoDyCoe::Finalize(){
  ...
  fBgoDyCoe->StopTime = fEvtHeader->fSecond;
